...
模块是 Faasit 代码的基本单位，定义了各种实体，如类型、值、块和装饰器。

`import`子句允许模块使用它们的唯一标识符引用其他模块。

导入分辨率的工作方式类似于 Go，在同一目录中将所有文件视为单个模块。

模块标识符以`std/sema`格式定义。

值得注意的是，有一个特殊的模块标识为`__main__`，它是 Faasit 编译器的入口模块，也
是插件处理的入口。
...
// 我们可以使用模块中的`use`子句来别名符号，别名可以在此模块中使用或导出到其他模块。
use {
